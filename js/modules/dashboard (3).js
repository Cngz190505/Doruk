...
async function fetchSparkline(symbol) {
  ...
  // Önce varsa eski seriyi anında göster; ardından güncel seri API'den yenilenir.
  try {
    const cached = JSON.parse(localStorage.getItem('cache_spark_'+symbol) || 'null');
    if(!_sparkCache[symbol] && cached && Array.isArray(cached.prices)) {
      _sparkCache[symbol] = cached.prices;
      drawSparkline(symbol, cached.prices);
    }
  } catch(e) {}
  // Canlı fiyat grafiği için API'deki güncel 24 saatlik seri her görünürlükte yenilenir.
  try {
...
// WebSocket'ten gelen fiyatı grafiğin son noktasına anında uygula.
// Tarihsel 24 saatlik seri korunur; yalnızca güncel kapanış noktası hareket eder.
function updateSparklineLive(symbol, price) {
  const value = Number(price);
  const series = _sparkCache[symbol];
  if (!Number.isFinite(value) || !Array.isArray(series) || series.length < 2) return;
  series[series.length - 1] = value;
  drawSparkline(symbol, series);
}

...
window._googleIleGirisYap = function() {
  ...
  function devamEt() {
    ...
    // Mobilde popup güvenilmez çalışıyor (popup-closed-by-user) — sayfa yönlendirmesi kullanıyoruz.
    // Sonucu, sayfa Google'dan dönünce _googleRedirectSonucuIsle() işler.
    localStorage.setItem('doruk_google_redirect_pending', '1');
    window.firebase.auth().signInWithRedirect(provider).catch(function(err) {
      localStorage.removeItem('doruk_google_redirect_pending');
      var mesaj = err && err.message ? err.message : String(err);
      showErr('Google girişi başlatılamadı: ' + mesaj);
    });
  }
...
function _initApp(onReady) {
  ...
  setTimeout(function() {
    ...
    // Watchlist — Firebase'den gelince fiyat + grafik bekle, hepsi hazır olunca splash kapat
    watchListLoad(function() {
      _splashStep(65, 'FİYATLAR ALINIYOR...');
      renderGrid();
      // Tüm crypto coinler için fiyat + sparkline paralel bekle
      ...
      // Fiyat + yüzde + sparkline hepsi gelsin
      ...
      var sparkPromises = cryptos.map(function(w){ return fetchSparkline(w.id); });
      // Binance/sparkline için de 5sn timeout — API yavaşsa splash takılmasın
      var _priceTimeout = new Promise(function(resolve) { setTimeout(resolve, 5000); });
      Promise.race([
        Promise.all(pricePromises.concat(sparkPromises)),
        _priceTimeout
      ]).then(function() {
        _splashStep(95, 'HAZIR!');
        setTimeout(function() { if(typeof onReady === 'function') onReady(); }, 200);
...
// Google yönlendirmeden dönüldüyse önce sonucu işle, sonra normal akışa devam et
function _googleRedirectSonucuIsle(cb) {
  function afterLoaded() {
    window.firebase.auth().getRedirectResult().then(function(result) {
      if (result && result.user) {
        ...
        db.ref('kullanicilar/' + kulKey).once('value', function(snap) {
          ...
          db.ref('banlar/' + kulKey).once('value', function(banSnap) {
            if (banSnap.exists()) {
              var er = document.getElementById('loginErr');
              if (er) er.textContent = '🚫 Hesabınız yasaklandı. Giriş yapamazsınız.';
              cb();
              return;
            }
            localStorage.setItem('doruk_login_user', kullaniciAdi);
            if (localStorage.getItem('doruk_misafir') !== '1') localStorage.removeItem('doruk_misafir');
            cb();
          });
        });
      } else {
      ...
      var er = document.getElementById('loginErr');
      if (er) er.textContent = 'Google girişi başarısız: ' + (err && err.message ? err.message : err);
      cb();
    });
  }
  if (!window.firebase || !window.firebase.auth) {
    function loadFbScript(src, done) {
      var s = document.createElement('script');
    ...
    if (!window.firebase) {
      loadFbScript('https://www.gstatic.com/firebasejs/9.23.0/firebase-app-compat.js', function() {
        loadFbScript('https://www.gstatic.com/firebasejs/9.23.0/firebase-auth-compat.js', afterLoaded);
      });
    } else {
      loadFbScript('https://www.gstatic.com/firebasejs/9.23.0/firebase-auth-compat.js', afterLoaded);
    }
...
document.addEventListener('DOMContentLoaded', () => {
  if (localStorage.getItem('doruk_google_redirect_pending') === '1') {
    localStorage.removeItem('doruk_google_redirect_pending');
    _googleRedirectSonucuIsle(_domReadyDevam);
  } else {
    _domReadyDevam();
  }
});

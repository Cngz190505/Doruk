...
async function syncRates() {
  try {
    const [rAltin, rDoviz] = await Promise.all([
      fetch('https://api.genelpara.com/json/?list=altin&sembol=GA,C,Y,T,CMR,ATA,14,18,22,XAUUSD,GAG', {mode:'cors',credentials:'omit'}),
      fetch('https://api.genelpara.com/json/?list=doviz&sembol=USD,EUR,GBP,CHF,JPY,SAR,AED,RUB,CAD,AUD,CNY,KWD', {mode:'cors',credentials:'omit'})
    ]);
    const dAltin = await rAltin.json();
    const dDoviz = await rDoviz.json();
    if(dAltin.success) gpData.altin = dAltin.data || {};
    if(dDoviz.success) gpData.doviz = dDoviz.data || {};
...
`;
  ...
  if(cryptoItems.length > 0) {
    ...
    if(unloaded.length > 0) Promise.all(unloaded.map(w => fetchSparkline(w.id)));
  }
